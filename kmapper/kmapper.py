...
class KeplerMapper(object):
    ...
    * {margin: 0; padding: 0;}
    ...
    h1 { %s padding: 20px; color: #fafafa; text-shadow: 0px 1px #000,0px -1px #000; position: relative; font: 300 30px Roboto, Sans-serif;}
    ...
    .meta {
      position: absolute; top: 160px; left: 20px; width: 220px;
      opacity: 0.9;  display: block; %s background: #000; line-height: 25px; color: #fafafa; border: 20px solid #000; font: 100 16px Roboto, Sans-serif;
    }

    .export_invalid {
      position:absolute; top: 80px; left: 20px; width: 220px;
      opacity: 0.9; display: block; background: #000; line-height: 25px; color: #fafafa; font: 100 16px Roboto, Sans-serif;
      padding: 18px; border: 2px solid #000000;
    }
    .export_valid {
      position:absolute; top: 80px; left: 20px; width: 220px;
      opacity: 0.9; display: block; background: #000; line-height: 25px; color: #fafafa; font: 100 16px Roboto, Sans-serif;
      padding: 18px; border: 2px solid #fafafa;
    }
    .export_invalid_hover {
      position:absolute; top: 80px; left: 20px; width: 220px;
      opacity: 0.25; display: block; background: #000; line-height: 25px; color: #fafafa; font: 100 16px Roboto, Sans-serif;
      padding: 18px; border: 2px solid #000000;
    }
    .export_valid_hover {
      position:absolute; top: 80px; left: 20px; width: 220px;
      opacity: 1.0; display: block; background: #000; line-height: 25px; color: #fafafa; font: 100 16px Roboto, Sans-serif;
      padding: 17px; border: 3px solid #fafafa;
    }
    ...
    <div id="holder">
      <h1>%s</h1>
      <p id="export_button" class="export_valid">
    ...
    // Mouseover of the export selection button to make it clear if it's valid
    d3.select('#export_button')
      .on("mouseover", function(d) {
        if(d3.select(this).classed("export_valid")) {
          d3.select(this).attr("class", "export_valid_hover");
        } else {
          d3.select(this).attr("class", "export_invalid_hover");
        }
      })
      .on("mouseout", function(d) {
        if(d3.select(this).classed("export_valid_hover")) {
          d3.select(this).attr("class", "export_valid");
        } else {
          d3.select(this).attr("class", "export_invalid");
        }
     });
